...
Set `APIDOT_KNOWN_TASK_IDS=task-unified-example` before running the server, or replace the in-memory Set with your own database lookup.
...
- Reconcile the task with `GET /api/generate/status/{task_id}` before irreversible business actions.
