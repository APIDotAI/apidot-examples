# Tripo H3.1 3D cURL Quickstart

## What this example shows

This example shows how to submit a Tripo H3.1 3D task through APIDot, store the returned `task_id`, and poll the shared status endpoint for the generated model file.

It includes the documented request shapes:

- `tripo3d-h3.1-text-to-3d` for text-to-3D.
- `tripo3d-h3.1-image-to-3d` for single-image-to-3D.
- `tripo3d-h3.1-multiview-to-3d` for multiview-to-3D.

## When to use it

Use this example when you need a server-side cURL quickstart for generating a 3D asset from text, one reference image, or multiple views.
...
- Public reference image URLs when using image-to-3D or multiview-to-3D.
...
curl --fail-with-body --request POST \
  ...
  --data '{
    ...
    "input": {
      "prompt": "A high-detail brass and leather fantasy compass with engraved symbols, polished metal rim, worn leather case, game asset",
      "negative_prompt": "low detail, broken geometry, extra parts",
      "face_limit": 100000,
      "texture": true,
...
- Passing a multiview request with too few or mismatched views.
- Assuming every Tripo 3D model accepts the same fields.
...
- Related landing page: https://apidot.ai/models/tripo-h31-3d

<!-- devrel:internal
## Promotion

- Topic: Tripo H3.1 3D generation with APIDot
- Audience: Developers building text-to-3D, image-to-3D, and multiview asset workflows
- Tweet angle: Show Tripo H3.1 3D through the APIDot async task flow and its documented request variants.
- Landing page: https://apidot.ai/models/tripo-h31-3d
